...
//                }
    ...
    @IBAction func onAPIkeyTapped(_ sender: UIButton) {
        
       
            let alertController = UIAlertController(title: "Enter New API Key", message: nil, preferredStyle: .alert)
            ...
            let saveAction = UIAlertAction(title: "Save", style: .default) { _ in
                if let newAPIKey = alertController.textFields?.first?.text {
                    // Save the new API key to the device (e.g., UserDefaults)
